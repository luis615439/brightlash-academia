...
# Cargar variables de entorno desde .env (relativo al archivo, no al cwd)
...
class AgentRouter:
    ...
    def respond(self, lead_id: str, message: str, script_context: str = None) -> RouterResponse:
        ...
        # 2. Transición NEW → QUALIFYING automática
        if lead.lead_state == LeadState.NEW:
            lead.state = LeadState.QUALIFYING.value

        # 3. Agregar mensaje del lead al historial
        lead.history.append({"role": "user", "content": message})

        # 4. Guardar ANTES de la API call (evita perder datos si falla)
        self.store.save(lead)

        # 5. Seleccionar agente con reglas explícitas
        agent = self._select_agent(lead)
        ...
        # 10. Agregar respuesta al historial y persistir
        ...
        return RouterResponse(
            ...
            agent_type=final_agent.agent_type.value,
            state_before=state_before,
            state_after=lead.state,
            segment=lead.segment,
            transition_occurred=transition_occurred,
            dry_run=self.dry_run,
            model_used=final_agent.model if not self.dry_run else "dry-run",
            tokens_used=tokens,
        )
