...
class C:
    ...
    # WHITE removido para usar el color por defecto de la terminal (evita invisibilidad en fondo blanco)
    GRAY    = "\033[30m"      # Changed from 90m (Bright Black) to 30m (Standard Black) to prevent bright-bleed
    GREEN   = "\033[32m"      # Changed from 92m
    RED     = "\033[31m"      # Changed from 91m
    YELLOW  = "\033[33m"      # Changed from 93m
    CYAN    = "\033[36m"      # Changed from 96m
    BLUE    = "\033[34m"

def gold(text: str) -> str:
    return f"{C.GOLD}{C.BOLD}{text}{C.RESET}"

def default_text(text: str) -> str:
    # Retornamos el texto crudo. En la terminal de Mac (fondo claro),
    # usar C.BOLD convierte el texto por defecto (negro) en "Bright Black" (gris tenue).
    return text

def dim(text: str) -> str:
    # Eliminamos el formato para evitar que la terminal de Zed se quede "pegada" en color gris tenue
    return text
...
def run_simulator(lead_id: str, router: AgentRouter) -> None:
    ...
    print(f"\n{dim('Simula una conversación de WhatsApp con JustLash Academy.')}")
    print_commands()
    print(SEPARATOR)
    ...
    while True:
        ...
        if user_input.lower() == "/reset":
            confirm = input(
                f"{yellow('¿Seguro que quieres reiniciar este lead? (s/N): ')}"
            ).strip().lower()
            if confirm == "s":
        ...
        if lead and lead.is_terminal:
            ...
            print(dim("  Usa /reset para reiniciar o /leads para ver otros leads."))
            continue
        ...
        try:
            ...
            if response.state_after == LeadState.CONVERTED.value:
                ...
                print(dim("  Usa /reset para simular otro lead o /exit para salir."))
            elif response.state_after == LeadState.DEAD.value:
                print(red("💀 Lead descartado después de 3 intentos de remarketing."))
                print(dim("  Usa /reset para reiniciar o /exit para salir."))

        except RuntimeError as e:
            print(red(f"\n❌ Error al conectar con OpenRouter:\n  {e}"))
            print(dim("  Verifica tu API key en el archivo .env"))
